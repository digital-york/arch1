class LandingPageController < ApplicationController
  ...
  def index
    ...
    SolrQuery.new.solr_query('has_model_ssim:Register', 'id, reg_id_tesim, title_tesim', 20)['response']['docs'].map do |result|
      id = result['id']
      reg_id_tesim = result['reg_id_tesim'][0]
      title_tesim = result['title_tesim'][0]
      @reg_id_tesim_list.each_with_index do |title, index|
        if title.match(reg_id_tesim)
          @reg_id_list[index] = id
          @reg_title_list[index] = title_tesim.strip
