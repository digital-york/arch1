class EntriesController < ApplicationController
  ...
  def index
    ...
    set_folio_and_image_drop_down if params[:set_folio] == 'true'
    ...
    if session[:folio_id] != ''
      ...
      set_folio_and_image(params[:small_zoom_action], session[:folio_id]) unless params[:small_zoom_action].nil?
  ...
  def edit
    ...
    @db_entry.db_related_places.each do |related_place|
      temp = []
      unless related_place.db_place_as_writtens[0].nil?
    ...
    @db_entry.db_related_agents.each do |related_person|
      temp = []
      unless related_person.db_person_as_writtens[0].nil?
  ...
  def create
    ...
    entry_params['entry_dates_attributes']&.each do |p|
      ...
      if entry_params['entry_dates_attributes'][p[0]]['single_dates_attributes'].nil? || (entry_params['entry_dates_attributes'][p[0]]['single_dates_attributes'] == '')
        next
      end
  ...
  def destroy
    ...
    SolrQuery.new.solr_query('continues_on_tesim:' + session[:folio_id], 'id', 1, 'entry_no_si asc')['response']['docs'].map do |_result|
      entry_count = SolrQuery.new.solr_query('folio_ssim:' + session[:folio_id], 'id', 100, 'id asc')['response']['docs'].map.size
      can_delete = false if entry_count <= 1
    end
  ...
  def whitelist_entry_params
    params.require(:entry).permit(
      :folio, :entry_no, :entry_id, :summary, rdftype: [], entry_type: [], section_type: [], marginalia: [], language: [],
                                              subject: [], note: [], editorial_note: [], is_referenced_by: [],
                                              entry_dates_attributes: [:id, :_destroy, :date_id, :date_role, :date_note, rdftype: [],
                                                                                                                         single_dates_attributes: [:id, :_destroy, :single_date_id, :date, :date_type, rdftype: [], date_certainty: []]],
